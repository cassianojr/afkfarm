...
const bot_creator = ({ username, pass, home, auth }) => {
  ...
  const mexeACabeca = async (bot, botLookingUp) => {
    if (bot.location === 'home') {
      bot.setControlState((botLookingUp)?'forward':'back', true);
      await bot.waitForTicks(50);
      bot.setControlState((botLookingUp)?'forward':'back', false);
      
      mexeACabeca(bot, !botLookingUp);
    }
  }
  ...
  bot.on('kicked', (reason, loggedIn) => {
    if (reason.includes("Vantagem Injusta")) {
      console.log('[!] tomou AC');
      return;
    }
